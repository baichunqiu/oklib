...
public class Request<B, E> implements Serializable {
    ...
    public <R> Request requestAgain() {
        switch (type) {
            case GET:
                if (isArr()) {
                    return NetKit.getArr(tag, url, params, parser, (IListCallback<R>) iCallback);
                } else {
                    return NetKit.get(tag, url, params, parser, (IBusiCallback<Integer, String>) iCallback);
                }
            case POST:
                if (isArr()) {
                    return NetKit.postArr(tag, url, params, parser, (IListCallback<R>) iCallback);
                } else {
                    return NetKit.post(tag, url, params, parser, (IBusiCallback<Integer, String>) iCallback);
                }
        }
        return null;
    }
